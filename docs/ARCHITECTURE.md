...
| unattributable red parent (nothing parseable) | result-level `unattributable` flag | warn: printed as inconclusive; enforce: fails the wave closed |
...
| a run needs the developer's checkout (dirty tree, their own branch, live editor) | `plan-load --worktree`: the run gets a linked worktree at `<project>/.swarmflow/worktrees/<name>` on the run branch and *that* checkout becomes the run root; the project store keeps a pointer to it (`runstate.resolve_project`) so either path reaches the run | the project's branch, index, and working tree are untouched apart from the standard `.gitignore` entries and the informational mirror (verified by comparing `git status --porcelain` around the run); the pointer is cleared when a later run is rooted at the project itself, and a pointer whose target disagrees is ignored |
| a per-task change is lost or unusable for review / rollback | `git.commit_tasks` (default on): after the wave's scope audit, one commit per delivered task on the run root's branch (brownfield preflight creates `swarmflow/<name>`; a greenfield scaffold commits on its own branch) containing that task's owner files as git sees them | `report["commits"]` and `commit`/`commit-skipped`/`commit-failed` ledger events; the committed set is literal-pathspec, excludes gitignored owner paths, and never touches the operator's other staged work |
| a task delivered on a retry would commit only its last attempt | the committed set is computed from HEAD (`changed_against_head`), not from the attempt's diff | the whole task lands in its commit; nothing of it is left dirty |
| a failed scope audit would commit misattributed work | the commit stage is skipped entirely when the audit fails | nothing is committed; the reason is printed and `report["commits"]` is empty |
