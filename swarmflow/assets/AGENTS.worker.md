...
4) ENVIRONMENT: npm/npx only (never pnpm/yarn/bun). Never inspect node_modules.
   Never install dependencies. Never start long-running servers or watchers (dev
   servers, preview servers, --watch): a test server must come from the injected
   verification command. If the same failure persists after 3 fix attempts,
